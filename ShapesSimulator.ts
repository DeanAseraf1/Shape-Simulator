...
let shapeChoise: string;
let shapeWidth: number;
let shapeHeight: number;
let contentChoise: string;
...
function NumberIsPositive(num: number): boolean {
    return num > 0;
}

//Validation Function to check if number is in a array
function StringIsInArray(str: string, list: string[]): boolean {
...
function CreateShape() {
    switch (shapeChoise) {
        ...
        case "1":
        case "2":
            for (let i: number = 1; i <= shapeHeight; i++) {
                for (let j: number = 1; j <= shapeWidth; j++) {
...
function PrintContent(j: number, i: number, jMax:number): void {
